...
func RegisterRoutes(server *gin.Engine, injector *do.Injector) {
	...
	userRoutes := server.Group("/api/user")
	{
		...
		userRoutes.PUT("/me", middlewares.Authenticate(jwtService), userController.Update)
		userRoutes.DELETE("/me", middlewares.Authenticate(jwtService), userController.Delete)
	}
}
